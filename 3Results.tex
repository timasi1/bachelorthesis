...
In this chapter, we analyze four different magnetic textures for their electronic-structure properties. The numerical results are compared with our symmetry-based predictions. The physically trivial cases, in which the hopping parameter set for one facing triangle is zero, e.g., $\lambda=t=0$, are not discussed due to the Hamiltonian being independent of $\mathbf{k}$, seen by using $t=\lambda=0$ in Eq.\eqref{eq: bloch-Hamiltonian} leading to a vanishing Berry curvature and flat bands. This stems from the electrons not being able to propagate in the full lattice due to a decomposition of the lattice into disconnected unit cells. 
The nonbreathing limit and the breathing case are analyzed separately. Note that the case $t'=0$ and $t\neq 0$, or vice versa $t=0$ and $t' \neq 0$, are already included in the breathing lattice and therefore are not analyzed separately.
\section{The S-type altermagnet \ce{Mn3Pt} with its T1 and T2 phases}

\subsection{Magnetic structure of the T1 and T2phases of \ce{Mn3Pt}}
...
The manganese atoms occupy the face-centered sites, and a $(111)$ plane forms a kagome lattice  of manganese atoms. The contribution of the platinum atoms is approximated as zero because the magnetic moments reside primarily on the manganese atoms, where T1 and T2 phases occur as defined in Ref.~\cite{Quelle3} and depicted in Fig.~\ref{fig: magnetic texture Mn3Pt}. For the T1 phase, the sublattice angles are $\varphi_{1,A} = \pi/2$, $\varphi_{1,B} = 7\pi/6$ and $\varphi_{1,C}= 11\pi/6$. The magnetic order of the T2 phase is equivalent to the order of the T1 phase with an additional global phase of $\pi/2$.
\begin{figure}[thb]
    \centering
    % First Subfigure
...
Additionally, the symmetries introduced in Section~\ref{section: Symmetries}, namely the $\Theta_s[C_{2z}||E]$ and $[E||C_{2z}]$-symmetries, are expected to hold in \ce{Mn3Pt}. The former forces the energy and in-plane spins to be even in $\mathbf{k}$, while forcing out-of-plane spins and the Berry curvature to be odd, holding in the absence of SOC. The latter holds in the nonbreathing limit, implying that the energy, spin and Berry curvature are even in momentum. \\
...
\end{align}
Here, the bands related by the symmetries may be equal $n=m$. This symmetry forces the in-plane net magnetization of both phases of \ce{Mn3Pt} to vanish. Therefore, we expect an in-plane antiferromagnetic behavior with spin-split bands. \\
Furthermore, $[C_{2\hat{s}}||\sigma_v]$ requires the out-of-plane net magnetization $M_z$ to vanish. Because this symmetry breaks for the T1 phase in the case of SOC, this phase may acquire an an out-of-plane magnetization in the presence of SOC. 

In contrast to this, this symmetry holds for SOC in the T2 phase, implying $M_z=0$. Hence, the T2 phase has a zero net magnetization ($M=0$), even in the presence of SOC. Because the in-plane magnetization vanishes for both phases, both altermagnets can be classified as S-type altermagnets, as introduced in Ref.~\cite{Cheong2025}. 
...
Figs.~\ref{fig:Mn3Pt1-nonbreathing} (a) and (b) show the computed band structures and the in-plane Fermi-surface spin textures for the non-breathing limit, whereas Figs.~\ref{fig:Mn3Pt1-breathing} (a) and (b) show it for a breathing kagome lattice. 
...
Note that we did not show that $n \neq \ell$ or $n\neq m$ and therefore the symmetry arguments do not prove that this degeneracy occurs. They only provide an explanation for possible occurring degeneracies at the given $\mathbf{k}$-points. \\


In contrast, we expect for the breathing kagome lattice the $[E||C_{2\hat{z}}]$-symmetry to be broken and therefore expect the energy, spin and Berry curvature not to be generally even in $\mathbf{k}$, as confirmed in Fig.~\ref{fig:Mn3Pt1-breathing}. However, in the absence of SOC, $\Theta_s[C_{2z}||E]$ still forces the in-plane spins and energies to be even and the Berry curvature to be odd in $\mathbf{k}$, observed in Fig.~\ref{fig:Mn3Pt1-breathing}. Therefore, even when $[E||C_{2\hat{z}}]$ is broken, even energy and in-plane-spins are still observed in the absence of SOC. Additionally, the out-of-plane spin component is expected to be odd in $\mathbf{k}$.\\

We also observe a $C_{3z}^s$-symmetry as expected.

!!!!!!!!!!!!!!!!!!! TM SOC?

Lastly, we observe $[C_{2\hat{s}}||\sigma_v]$ for no SOC and the $\mathcal{TM}$ planes, depicted in Fig.~\ref{fig: magnetic texture Mn3Pt} (a). The $\mathcal{TM}$-typical behavior is given by Eq.~\eqref{eq: symmetry_TM} and is also observed under in extensive numerical calculation in the presence of SOC. Therefore, we expect the material to have conserved $\mathcal{TM}$ planes. This needs to be shown under a further study according to Eq.~\eqref{eq: TM symmetry prove eq}.  
...
A similar behavior in comparison to the T1 phase is observed. The principal difference is that the T2 phase has unprimed $\mathcal{M}$-planes rather than the $\mathcal{TM}$ planes of T1 and an additional global phase shift, as shown in Fig.~\ref{fig: magnetic texture Mn3Pt} (b). 
\\
...
The alternating form of the Berry curvature for a breathing lattice comes from the combination of $C_{3z}^s$ and the mirror plane $\mathcal{M}$. An odd behavior of the Berry curvature is not expected, and can be checked numerically, yielding
\begin{align}
...
In this section, the model and symmetries formulated in Chapter~\ref{chapter:Theory} are applied to the magnetic texture of the M-type altermagnets \ce{Mn3Ge} and \ce{Mn3Sn}. Further, the resulting expectations are compared with the results by computing the band structure, in-plane Fermi-surface spin texture and the out-of-plane component of Berry curvature.
...
The magnetic moments form a triangular $120^\circ$ order and therefore the structures depicted in Figs.~\ref{fig: magnetic texture Mn3Ge} and \ref{fig: magnetic texture Mn3Sn}, follow from the magnetic point groups $m'mm'$ or $mm'm'$, respectively.\\
The phases of the magnetic texture in Fig.~\ref{fig: magnetic texture Mn3Ge and Sn} are for \ce{Mn3Ge} $\varphi_{3,A} = \pi/2$, $\varphi_{3,B} = 11\pi/6$ and $\varphi_{3,C}= 7\pi/6$. The magnetic texture of \ce{Mn3Sn} is obtained by adding a global spin rotation of $\pi/2$ to the magnetic moments.\\
...
The $\mathcal{TM}$ and $\mathcal{M}$ planes depicted in Fig.~\ref{fig: magnetic texture Mn3Ge and Sn} force the properties in Eq.~\eqref{eq: symmetry_M} and \eqref{eq: symmetry_TM}. !!!!!!!!!. 
The mirror plane $\mathcal{M}$, which is in this case equal to $[C_{2\hat{s}}||\sigma_v]$ because of a perpendicular spin to the mirror plane, is expected to hold in the presence of SOC, due to $[C_{2\hat{s}}||\sigma_v]$ transforming spin-space and real-space equivalently.
...
These symmetries constrain the direction of an allowed magnetization. When they are broken by adding SOC, a net magnetization is permitted in \ce{Mn3Ge} within the $\mathcal{TM}$ plane, i.e., along the $y$-axis, and in \ce{Mn3Sn} perpendicular to the $\mathcal{M}$ plane, i.e., along the $x$-axis.  
...
\end{figure}
For the nonbreathing limit, an even Berry curvature, energies and spins are observed in $\mathbf{k}$, as imposed by $[E||C_{2z}]$, which also holds in the presence of SOC.\\
...
The effects of the resulting net magnetization can be observed by the Fermi-surface spin texture in Figs.~\ref{fig:Mn3Ge-nonbreathing} and \ref{fig:Mn3Ge-breathing} in the case of SOC. We next discuss the origin of the deformation of the Fermi-surface. The symmetry plane constrains spins along $k_y=0$ to align along $y$ or with the net magnetization. Using the Rashba term, according to Eq.~\eqref{eq: Rashba_Hamiltonian_standard}, first-order Taylor expansion and calculating the energy by taking the expectation value with the eigenstates, we obtain
\begin{align}
...
The Figs.~\ref{fig:Mn3Ge-nonbreathing} and \ref{fig:Mn3Ge-breathing} show that the bands are spin-split also in the absence of SOC, classifying \ce{Mn3Ge} as a strong altermagnet. The spin-splitting arises because the compensated texture breaking $\mathcal{PT}$ symmetry and other symmetries enforcing band degeneracies.
...
Note that one of the constraints imposed by the symmetry $[C_{3z}||C^{-1}_{3z}]$ according to Eq.~\eqref{eq: C_3z^-1 symm} can be observed in the presence of SOC, hence verifying that the found $[C_{3z}||C_{3z}^{-1}]$ symmetry breaks in the presence of SOC. 
\begin{figure}[htbp]
    \centering
    \includegraphics[
        width=\linewidth
