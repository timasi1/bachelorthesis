...
In this chapter, we analyze four different magnetic textures for their electronic-structure properties. The numerical results are compared with our symmetry-based predictions. The physically trivial cases, in which the hopping parameter set for one facing triangle is zero are not discussed due to the Hamiltonian being independent of $\mathbf{k}$. This can be seen by setting $t=\lambda=0$ in Eq.\eqref{eq: bloch-Hamiltonian}, making the Hamiltonian independent of $\mathbf{k}$, which produces a vanishing Berry curvature and flat bands. This stems from the electrons not being able to propagate in the full lattice due to a decomposition of the lattice into disconnected unit cells. 
The nonbreathing limit and the breathing case are analyzed separately. Note that the case $t'=0$ and $t\neq 0$, or vice versa $t=0$ and $t' \neq 0$, is already included in the breathing lattice and therefore are not analyzed separately.
...
\begin{figure}[thb]
        ...
        \begin{tikzpicture}[scale=1.5]
          ...
          \draw[thick, brown] (1,{4*sqrt(3)/3}) -- (1,{-4*sqrt(3)/3}) node[below, font=\small]{$\mathcal{M}_1$, $\sigma_{v,1}$};
            \draw[thick, brown] (3, {-2*sqrt(3)/3}) -- (-1, {2*sqrt(3)/3}) node[above right,  xshift= -20pt, font=\small]{$\mathcal{M}_3$, $\sigma_{v,3}$};   
             ...
             \draw[thick, purple, dashed] (0, {-3*sqrt(3)/3}) -- (2, {3*sqrt(3)/3}) node[above left,  xshift= 20pt, font=\small]{$\mathcal{TM}_2$};
          \draw[thick, purple, dashed] (0,{3*sqrt(3)/3}) -- (2,{-3*sqrt(3)/3}) node[below, font=\small]{$\mathcal{TM}_1$};
            \draw[thick, purple, dashed] (3, {0}) -- (-1, {0}) node[above right,  xshift= -20pt, font=\small]{$\mathcal{TM}_3$};     
          % sublattice A
          \foreach \x/\y in {
            1/{-\rt},
            0/0,
    ...
    \caption{T2}
    \end{subfigure}
    \caption{The magnetic order of the T1 and T2 phases of \ce{Mn3Pt} with the illustrated conserved time-reversal mirror plane $\mathcal{TM}_i$ or mirror plane $\mathcal{M}_i$, respectively and the reflection symmetry planes $\sigma_{v,i}$, belonging to the $[C_{2\hat{s}}||\sigma_v]$ operation, depicted on the corresponding kagome lattice. The T1 order has time-reversal mirror planes, while the T2 phase has mirror planes as well as time-reversal mirror planes. The $\mathcal{TM}_i$ planes of the T2 phase only hold in the nonbreathing limit.}
...
By contrast the $\mathcal{TM}$ plane may break for SOC, as has to be shown in a further study, where Eq.~\eqref{eq: TM symmetry prove eq} has to be proven with the Hamiltonian of Eq.~\eqref{eq: bloch-Hamiltonian} for the magnetic texture of the T1 phase. 
We also find $\mathcal{TM}$ symmetries for the T2 phase, which only hold in the nonbreathing limit due to them switching upward and downward-facing triangles. This also has to be shown by a future work, analogously to before. It might be intuitive to also add $\mathcal{M}$ planes to the T1 phase, however these
!!!!!!!!
...
Because the T1 and T2 magnetic orders differ by a global spin rotation of $\pi/2$, equivalent results are expected in the absence of SOC with respect to the phase. With SOC, the phase difference acts differently on the Rashba Hamiltonian and therefore leads to different results.\\
...
For the breathing kagome lattice, we expect the $[E||C_{2\hat{z}}]$-symmetry to be broken and therefore the energy, spin and Berry curvature not to be generally even in $\mathbf{k}$, as confirmed in Fig.~\ref{fig:Mn3Pt1-breathing}. However, in the absence of SOC, $\Theta_s[C_{2z}||E]$ still forces the in-plane spins and energies to be even and the Berry curvature to be odd in $\mathbf{k}$, observed in Fig.~\ref{fig:Mn3Pt1-breathing}. Therefore, even when $[E||C_{2\hat{z}}]$ is broken, the energy and in-plane spins remain even in the absence of SOC. Additionally, the out-of-plane spin component is expected to be odd in $\mathbf{k}$.\\

We also observe a $C_{3z}^s$-symmetry as expected.

!!!!!!!!!!!!!!!!!!! TM SOC?

Lastly, we observe $[C_{2\hat{s}}||\sigma_v]$ in the absence of SOC and the $\mathcal{TM}$ planes, depicted in Fig.~\ref{fig: magnetic texture Mn3Pt} (a). The $\mathcal{TM}$-typical behavior is given by Eq.~\eqref{eq: symmetry_TM} and is also observed in extensive numerical calculation in the presence of SOC. Therefore, we expect the material to have conserved $\mathcal{TM}$ planes. This needs to be confirmed theoretically in a future work according to Eq.~\eqref{eq: TM symmetry prove eq}.  
...
\\
Similar symmetry arguments for the occurrence of degeneracies, as given in Eqs.~\eqref{eq: degeneracy_C3z_even_for_K},~\eqref{eq: degeneracy_even_for_Gamma} and \eqref{eq: degeneracy_even_for_M}, can be applied to the T2 phase, and therefore degeneracies in $K$, $\Gamma$ and $M$ are permitted if the energy is even in $\mathbf{k}$.
...
Thus, the Berry curvature vanishes in the absence of SOC in the nonbreathing limit. We also observe, that the Berry curvature vanishes in the presence of SOC, and thus the Berry curvature has to be odd also for SOC. This can be argued by using the $\mathcal{M}$ plane in combination of the $\mathcal{TM}$ plane depicted in Fig.~\ref{fig: magnetic texture Mn3Pt}(b), which holds only in the nonbreathing limit, leading to an odd Berry curvature. Note that we here assume that the $\mathcal{TM}$ plane holds also for SOC, which has to be shown in a future work. 
\begin{figure}[tbhp]
    \centering
    \includegraphics[
        width=\linewidth
