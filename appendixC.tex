...
\end{figure}
Then, by computing the Fermi-surface spin texture for the resulting Hamiltonian in the nonbreathing limit, Fig.~\ref{fig:homogenous-nonbreathing} is obtained. Thus, the spins align parallel along the magnetic order for higher energies , as discussed in Section~\ref{section: tightbinding}, thereby confirming the introduced sign. Note that with SOC the spins acquire additional components, and hence they do not align anymore with the magnetic texture.\\
...
Now, by adding SOC with $\lambda'=t'=0$, we obtain three flat bands, occurring due to a broken orbital degeneracy. Note that the conserved $\mathcal{T}$ symmetry in the Rashba term enforces a Kramers degeneracy.
...
By adding a magnetic texture, the Kramers degeneracy and therefore lifting the degeneracy up to six distinct bands, due to the magnetic texture breaking time-reversal and the lack of an antiunitary symmetry prote
...
cting the degeneracy.

Furthermore, adding hopping on upward- and downward-facing triangles, so $t',t\neq0$ or $\lambda',\lambda\neq 0$, dispersive bands are obtained, as seen in Fig.~\ref{fig:homogenous-notexture} (b), (d). For no SOC and no magnetic texture, Kramers pairs are expected, resulting in three doubly degenerate bands, as observed.  These bands are split by adding SOC or a magnetic texture, therefore resulting in six dispersive bands.
...
The non-treated cases, for example breathing lattice with SOC, are not analyzed in detail because we expect up to six dispersive bands in all of them, in which degeneracies enforced by the lattice or magnetic symmetries may occur. 
